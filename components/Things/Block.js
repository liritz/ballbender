class Block extends Thing {
  constructor(id, position = [0,0], speed = [1,1], size = [30,30], color = 'rgba(255,0,0,1)') {
    ...
    this.type = `block`;
    this.id = this.id = `${this.type}_${id}`;
    this.size = {
      width: size[0],
      height: size[1]
    };
    this.hitpoints = this.size.height * this.size.width;
  ...
  reactTo(thing) {
    if (thing.type === 'ball') {
      this.hitpoints -= thing.mass;
      this.color = `rgba(255,0,0,${(3/4) * (this.hitpoints / this.mass) + (1 / 4)})`;
    }
  }
}