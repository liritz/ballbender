class Physics {
  constructor(space) {
    ...
    this.lightspeed = 10;    // nothing can be faster than this
    this.planktime = 16;     // smallest duration in milliseconds
    this.friction = 0.00001; // by how strongly does the speed get reduced every planktime (0 means no friction)
    this.G = 0.000004;       // officially it is 6.7 * 10^-11
    this.anti_G = this.G;    // the gravity constant of anti-gravity
  ...
  attract_CB(controller, ball) {
    ...
    ball.state.spe.add(this.gravity_pull(ball, controller));
  }
  ...
  bounce_BL(ball, wall) {
    // Change state of ball to non-collide first
    const corrector = wall.area.closestPointOnCenterLine(ball.state.pos).to(ball.state.pos).direction;
    while (this.collision_BL(ball, wall)) {
      ball.state.pos.add(corrector);
    }
  ...
  collision_BB(ball_1, ball_2) {
    // Check if the balls overlap (or will overlap?)
  ...
  // ?? Can we handle fireballs with reactTo? maybe not...
  ...
  gravity_pull(object_1, object_2) {
    const v_1 = object_1.area.closestPointTo(object_2.area.closestPointOnCenterLine(object_1.state.pos));
    const v_2 = object_2.area.closestPointTo(object_1.area.closestPointOnCenterLine(object_2.state.pos));
    const x_1 = object_1.state.pos;
    const x_2 = object_2.state.pos;
    const m = object_2.mass;
    if (x_1.distanceTo(x_2) > object_1.radius + object_2.radius) {
      return v_1.to(v_2).times((this.G * m) / x_1.distanceTo(x_2));
    }
    else {
      return v_1.to(v_2).times((this.anti_G * m) * x_1.distanceTo(x_2));
    }
  }
